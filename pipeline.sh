...
singularity -s exec -B ${REFERENCE_DIR}:/data -B ${OUTPUT_FULL}:/output image_files/bowtie2_2.2.8--py35_2.sif bowtie2-build /data/${REFERENCE_BASE} /output/bowtie2-index/${REFERENCE_BASE} > ${OUTPUT_FULL}/logs/01-bowtie2-index.stdout 2> ${OUTPUT_FULL}/logs/01-bowtie2-index.stderr || { echo 'bowtie2 index failed'; exit 1; }

### Map Reads to Reference Legionella pneumophila Philadelphia genome
mkdir -p ${OUTPUT_FULL}/map-sam

echo '02) bowtie2 align'
singularity -s exec -B ${R1_DIR}:/data1 -B ${R2_DIR}:/data2 -B ${OUTPUT_FULL}:/output image_files/bowtie2_2.2.8--py35_2.sif bowtie2 -x /output/bowtie2-index/${REFERENCE_BASE} --very-sensitive-local --no-unal -a -1 /data1/${R1_BASE} -2 /data2/${R2_BASE} -S /output/map-sam/${ISOLATE}.sam > ${OUTPUT_FULL}/logs/02-bowtie2-align.stdout 2> ${OUTPUT_FULL}/logs/02-bowtie2-align.stdout || { echo 'bowtie2 align failed'; exit 1; }

### Convert Sam to Bam File
mkdir -p ${OUTPUT_FULL}/map-bam

echo '03) samtools view'
singularity -s exec -B ${REFERENCE_DIR}:/data -B ${OUTPUT_FULL}:/output image_files/samtools_1.3.1--3.sif samtools view -bS -T /data/${REFERENCE_BASE} /output/map-sam/${ISOLATE}.sam -o /output/map-bam/${ISOLATE}.bam > ${OUTPUT_FULL}/logs/03-samtools-view.stdout 2> ${OUTPUT_FULL}/logs/03-samtools-view.stderr || { echo 'samtools view failed'; exit 1; }

echo '04) samtools sort'
singularity -s exec -B ${OUTPUT_FULL}:/output image_files/samtools_1.3.1--3.sif samtools sort -o /output/map-bam/${ISOLATE}-sorted.bam -O bam /output/map-bam/${ISOLATE}.bam > ${OUTPUT_FULL}/logs/04-samtools-sort.stdout 2> ${OUTPUT_FULL}/logs/04-samtools-sort.stderr || { echo 'samtools sort failed'; exit 1; }

echo '05) samtools index'
singularity -s exec -B ${OUTPUT_FULL}:/output image_files/samtools_1.3.1--3.sif samtools index /output/map-bam/${ISOLATE}-sorted.bam > ${OUTPUT_FULL}/logs/05-samtools-index.stdout 2> ${OUTPUT_FULL}/logs/05-samtools-index.stderr || { echo 'samtools index failed'; exit 1; }

mkdir -p ${OUTPUT_FULL}/variants

### Call SNPs with FreeBayes
echo '06) freebayes'
singularity -s exec -B ${REFERENCE_DIR}:/data -B ${OUTPUT_FULL}:/output image_files/freebayes_1.2.0--py27h56106d0_4.sif freebayes -q 20 -p 1 --min-coverage 20 -F 0.75 -j -f /data/${REFERENCE_BASE} /output/map-bam/${ISOLATE}-sorted.bam -v /output/variants/${ISOLATE}-freebayes-SNP-differences.vcf > ${OUTPUT_FULL}/logs/06-freebayes.stdout 2> ${OUTPUT_FULL}/logs/06-freebayes.stderr || { echo 'freebayes failed'; exit 1; }

### Use VCFTools to create a VCF File
echo '07) vcftools'
singularity -s exec -B ${OUTPUT_FULL}:/output image_files/vcftools_0.1.14--5.sif vcftools --vcf /output/variants/${ISOLATE}-freebayes-SNP-differences.vcf --remove-indels --recode --recode-INFO-all --out /output/variants/${ISOLATE}-freebayes-SNP-differences-NO-INDELS > ${OUTPUT_FULL}/logs/07-vcftools.stdout 2> ${OUTPUT_FULL}/logs/07-vcftools.stderr || { echo 'vcftools failed'; exit 1; }

### Use vcfFilter to remove low quality SNPs
echo '08) vcffilter'
singularity -s exec -B ${OUTPUT_FULL}:/output image_files/vcflib_1.0.0_rc2--h56106d0_1.sif vcffilter -f "QUAL > 1" /output/variants/${ISOLATE}-freebayes-SNP-differences-NO-INDELS.recode.vcf > ${OUTPUT_FULL}/variants/${ISOLATE}-vcffilter.vcf 2> ${OUTPUT_FULL}/logs/08-vcffilter.stderr || { echo 'vcffilter failed'; exit 1; }

### GZip VCF file
echo '09) bgzip'
singularity -s exec -B ${OUTPUT_FULL}:/output image_files/htslib_1.9--h47928c2_5.sif bgzip /output/variants/${ISOLATE}-vcffilter.vcf > ${OUTPUT_FULL}/logs/09-bgzip.stdout 2> ${OUTPUT_FULL}/logs/09-bgzip.stderr || { echo 'bgzip failed'; exit 1; }

### Use Tabix command on Gzipped VCF file
echo '10) tabix'
singularity -s exec -B ${OUTPUT_FULL}:/output image_files/htslib_1.9--h47928c2_5.sif tabix -f -p vcf /output/variants/${ISOLATE}-vcffilter.vcf.gz > ${OUTPUT_FULL}/logs/10-tabix.stdout 2> ${OUTPUT_FULL}/logs/10-tabix.stderr || { echo 'tabix failed'; exit 1; }
...
cat ${REFERENCE_FULL} | singularity -s exec -B ${OUTPUT_FULL}:/output image_files/vcftools_v0.1.15_cv2.sif vcf-consensus /output/variants/${ISOLATE}-vcffilter.vcf.gz > ${OUTPUT_FULL}/consensus/${ISOLATE}.fasta 2> ${OUTPUT_FULL}/logs/11-vcf-consensus.stderr || { echo 'vcf-consensus failed'; exit 1; }
...
singularity -s exec -B ${REFERENCE_DIR}:/data -B ${OUTPUT_FULL}:/output image_files/bedtools_2.23.0--hdbcaa40_3.sif bedtools genomecov -bga -split -ibam /output/map-bam/${ISOLATE}-sorted.bam -g /data/${REFERENCE_BASE} > ${OUTPUT_FULL}/consensus/${ISOLATE}-per-site-depth-coverage.txt 2> ${OUTPUT_FULL}/logs/13-bedtools-genomecov.stderr || { echo 'bedtools genomecov failed'; exit 1; }

### Use python script to mask sites below 25x depth coverage
echo '14) mask sites'
singularity -s exec -B ${OUTPUT_FULL}:/output image_files/lpserogroup_python_latest.sif python /scripts/maskSites.py /output/consensus/${ISOLATE}-one-line.fasta /output/consensus/${ISOLATE}-per-site-depth-coverage.txt /output/consensus/${ISOLATE}-masked-seq.fasta > ${OUTPUT_FULL}/logs/14-mask-sites.stdout 2> ${OUTPUT_FULL}/logs/14-mask-sites.stderr || { echo 'mask sites failed'; exit 1; }

### Use Bedtools to identify gene sequences and their start/end coordinates
echo '15) bedtools getfasta'
singularity -s exec -B ${GFF_DIR}:/data -B ${OUTPUT_FULL}:/output image_files/bedtools_2.26.0gx--he860b03_3.sif bedtools getfasta -fi /output/consensus/${ISOLATE}-masked-seq.fasta -bed /data/${GFF_BASE} -name -fo > ${OUTPUT_FULL}/consensus/${ISOLATE}-genes-LPS-regions.fasta 2> ${OUTPUT_FULL}/logs/15-bedtools-getfasta.stderr || { echo 'bedtools getfasta failed'; exit 1; }
...
singularity -s exec -B ${OUTPUT_FULL}:/output image_files/lpserogroup_python_latest.sif python /scripts/genePresenceAbsenceMatrix.py /output/consensus/${ISOLATE}-genes-LPS-regions.fasta ${ISOLATE} /output/features/${ISOLATE}-output-genes-LPS.txt > ${OUTPUT_FULL}/logs/15-gene-presence-absence-matrix.stdout 2> ${OUTPUT_FULL}/logs/15-gene-presence-absence-matrix.stderr || { echo 'gene presence absence matrix failed'; exit 1; }
...
ulimit -s 5248800
singularity -s exec -B ${OUTPUT_FULL}:/output image_files/lpserogroup_rscripts_latest.sif Rscript --max-ppsize=500000 /scripts/R_modelPredict_version_0.1_20180904Model.R /output/features/${ISOLATE}-sites-extract.csv ${ISOLATE} /output/predict/ > ${OUTPUT_FULL}/logs/18-r-model-predict.stdout 2> ${OUTPUT_FULL}/logs/18-r-model-predict.stderr || { echo 'r model predict failed'; exit 1; }
...
singularity -s exec -B ${OUTPUT_FULL}:/output image_files/lpserogroup_python_latest.sif python /scripts/mergePredictResults.py /output/predict/${ISOLATE}-predictResults.txt ${ISOLATE} /output/predict/${ISOLATE}-final-prediction-results.txt > ${OUTPUT_FULL}/logs/19-merge-predict-results.stdout 2> ${OUTPUT_FULL}/logs/19-merge-predict-results.stderr || { echo 'merge predict results failed'; exit 1; }
